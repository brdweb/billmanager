...
class TestLogin:
    ...
    def test_login_wrong_password(self, client, admin_user):
        ...
        response = client.post(
            "/login", json={"username": "testadmin", "password": "wrongpassword"}
        )
    ...
    def test_login_nonexistent_user(self, client):
        ...
        response = client.post(
            "/login", json={"username": "nonexistent", "password": "anypassword"}
        )
        assert response.status_code == 401
...
class TestPasswordChange:
    ...
    def test_v1_password_change_sets_session(self, client, app, db_session):
        ...
        with app.app_context():
            ...
            user = User(
                username="firstadmin", role="admin", password_change_required=True
            )
            user.set_password("initialpassword")
            db_session.add(user)

            # Create a database and grant access
            test_db = Database(name="testdb", display_name="Test Database")
            ...
            login_response = client.post(
                "/login", json={"username": "firstadmin", "password": "initialpassword"}
            )
            login_data = json.loads(login_response.data)
