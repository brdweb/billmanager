...
export function Settings() {
  ...
  const { isAdmin, user, updateCurrency } = useAuth();
  ...
  const currencyOptions = (config?.supported_currencies ?? ['USD']).map((currency) => ({
    value: currency,
          ...
          <Tabs.Panel value="settings" pt="lg">
            <Box maw={720}>
              <Stack gap="xl">
                ...
                <Select
                  label={t('settingsPage.currencyLabel')}
                  description={t('settingsPage.currencyDescription')}
                  leftSection={<IconCurrencyDollar size={16} />}
                  data={currencyOptions}
                  value={user?.currency ?? 'USD'}
                  onChange={(value) => value && void updateCurrency(value)}
                  allowDeselect={false}
                  searchable
                  w={340}
                />
